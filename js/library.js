...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildSidebar () {
    ...
    // Auto-expand the category matching the current hash
      ...
      return `
        <section class="lib-category-section reveal"
          ...
          <div class="lib-cat-header">
            <span class="lib-cat-icon" style="--cat-color:${cat.color}"
                ...
                <a href="project.html?cat=${cat.id}&id=${p.id}"
                  ...
                  <div class="lib-proj-card-top">
                    ...
                    <span style="font-family:var(--font-mono);font-size:.62rem;
                                 color:var(--tx-03)">.${p.type}</span>
                  </div>
                  <div class="lib-proj-title">${p.title}</div>
                  <p class="lib-proj-desc">${p.description}</p>
                  <div class="lib-proj-meta">
                    <span class="lib-proj-author">
                      ${MCL.getContributor(p.author)?.name.split(' ')[0] || p.author}
                    </span>
          ...
          ` : `
            <p style="font-family:var(--font-serif);font-size:.82rem;
                      color:var(--tx-03);padding:12px 0">
              No implementations match current filters in this category.
            </p>
          `}
        </section>
      `;
